...
export class ChartBaseMultiBarComponent extends ChartBaseComponent {
  ...
  draw () {
    ...
    this.xScaleInner = d3.scaleBand()
      .paddingInner(0.3)

    this.xScaleOuter = d3.scaleBand()
    ...
    this.xScaleInner.domain (this.keys)
        .rangeRound([0, this.xScaleOuter.bandwidth()])
    this.yScale.domain ([0, 1])

    this.xAxis
        .attr('transform', `translate(0, ${this.height})`)
        .call(d3.axisBottom(this.xScaleOuter)
                .tickValues(this.xScaleOuter.domain().filter(function (d, i) { return !(i % 3) }))
    ...
    this.rects = this.bar.selectAll('g')
            .data(this.data.keys, k => k.key)
            .enter().append('rect')
            .attr('width', xScaleInner.bandwidth())
            .attr('class', (d) => d.key)
            .attr('id', function (d) { return (values[d.key][this.parentNode.id] === null) ? 'null' : 'notnull' } )
            .attr('x', (d) => xScaleInner(d.key))
            .attr('y', function (d) { return yScale(values[d.key][this.parentNode.id]) })
            .attr('height', function (d) { return height - yScale(values[d.key][this.parentNode.id]) })

    this.rects.nodes().filter((d) => d.id === 'null')
                      .forEach(function (d) {
    ...
    this.bar.selectAll('rects')
           ...
           .attr('x', (d) => xScaleInner(d.key))
           .attr('y', yScale(1) )
           .attr('height', function (d) { return yScale(values[d.key][this.parentNode.id]) })
